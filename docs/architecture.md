...
Проект: «Idea Frame Livedemo». Документ фиксирует целевую архитектуру решения, ключевые компоненты, взаимодействия и потоки данных. На основе архитектуры будут подготовлены логическая модель данных (`db/schema.dbml`) и спецификация API (`docs/openapi.yaml`).
